...
def test(args, basedmodel, ppo, classifymodel, FusionHisF, memory, val_loader):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    try:
        model_dtype = next(basedmodel.parameters()).dtype
    ...
    with torch.no_grad():
        label_list = []
        Y_prob_list = []
        for idx, (coords, data, label) in enumerate(val_loader):
            update_coords, update_data, label = coords.to(device), data.to(device), label.to(device).long()
            update_data = update_data.to(model_dtype)
            ...
            for patch_step in range(0, total_T):
                ...
                action_index_pro, memory = grouping_instance.rlselectindex_grouping(ppo, memory, update_coords, sigma=0.02, restart=restart)  
